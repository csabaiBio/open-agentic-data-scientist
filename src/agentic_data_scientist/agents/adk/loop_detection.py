...
class LoopDetectionAgent(LlmAgent):
    ...
    def _parse_unknown_tool_error(self, exc: Exception) -> Tuple[bool, Optional[str]]:
        ...
        """
        msg = str(exc)
        # ADK old format: "Function <name> is not found in the tools_dict."
        match = re.search(r"Function\s+([\w\-.]+)\s+is\s+not\s+found\s+in\s+the\s+tools_dict", msg)
        if match:
            return True, match.group(1)
        # ADK new format: "Tool '<name>' not found."
        match2 = re.search(r"Tool\s+'([\w\-.]+)'\s+not\s+found", msg)
        if match2:
